    ...
    def clear(self, layer_id: Optional[str] = None, sample_indices: Optional[torch.Tensor] = None):
        ...
        if layer_id is None and sample_indices is None:
            ...
            for k in keys_to_remove:
                del self.cache[k]
        elif sample_indices is not None and layer_id is None:
            # Clear all layers for specific sample indices
            idx_set = set(sample_indices.tolist())
            keys_to_remove = [k for k in self.cache.keys() if k[1] in idx_set]
            for k in keys_to_remove:
                del self.cache[k]
